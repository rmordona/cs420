...
set.seed(142)
n = nrow(dataset)
ind = sample(1:nrow(dataset), n * 8/10, replace = FALSE, prob = NULL)
...
Basically, if we are to assume for a moment that the test data all come from our attempts to brew beer, it would seem that by following the measuring gadgets ( or sensors ) based on the preferred model 3, it can be observed that an accuracy of `r accuracy` seems to be enough to attain some level of alcohol content without other extraneous sensors - and to rely for more additional sensors does not give additional benefit or effect (see R-squared/RMSE) (though statistically, those extraneous sensors may be significant by ANOVA). It should be noted that based on the Exhaustive search (model 3), for ABV (alcohol by volume), by way of parameterization (sort of brute force), it suggest to have a good mix of either "extract" or "BIAB" for BrewMethod along with "Specific Gravity" for SugarScale. The interaction between Color and BoilTime, Boltime and Brewmethod of 'extract'. (This could very well be done also by subsetting dataset to those observations only)
...
***Additioinal Data Analysis for model 3:***

Below are some of the extra analysis done. **(May require more improvement)**

Notice that our Residual-vs-Fitted plot still shows a pattern. Also, the Q-Q box plot have tails that are moving away from the distribution line.  
...
# Data analysis on model 3:

# Shapiro.test requires smaller sample size upto 5000 only. Use the following below
small_index = sample(1:nrow(new_train_set), 4500)
smaller_sample = data_no_outliers[small_index, ]

# Now re-fit model 3
model3_small_sample = lm( ABV ~ OG + Color + v1 + v2 + v3 + OG:v1 + Color:BoilTime +  BoilTime:v3,
              data = smaller_sample)
#
# Shapiro Test
shapiro.test(resid(model3_small_sample))
#
# Leverage Analysis
sum(hatvalues(model3_small_sample) > 2 * mean(hatvalues(model3_small_sample)))
#
# Residual Analysis
sum(abs(rstandard(model3_small_sample)) > 2)
#
# Cooks Distance Analysis
#cd_mpg_hp_add = cooks.distance(model3_small_sample)
#sum(cd_mpg_hp_add > 4 / length(cd_mpg_hp_add))
#
#
par(mfrow = c(2, 2))
plot(model3_small_sample)

# Let us ignore Shapiro.test and use the actual training set with no outliers.
# Let's use on cooks distance again
big_mod_cd = cooks.distance(model3)
sum(big_mod_cd > 4 / length(big_mod_cd))
  
# Now re-fit model 3

model3_extra_analysis = lm( ABV ~ OG + Color + v1 + v2 + v3 + OG:v1 + Color:BoilTime +  BoilTime:v3,
              data = data_no_outliers, subset = big_mod_cd < 4 / length(big_mod_cd))

par(mfrow = c(2, 2))
qqnorm(resid(model3), col = "darkgrey")
qqline(resid(model3), col = "dodgerblue", lwd = 2)
qqnorm(resid(model3_extra_analysis), col = "darkgrey")
qqline(resid(model3_extra_analysis), col = "dodgerblue", lwd = 2)

# Plot residual-fitted, and return dataset without outliers.
data_no_outliers =  residual_fitted_plot(model3, data_no_outliers, FALSE, 0) 
data_no_outliers =  residual_fitted_plot(model3_extra_analysis, data_no_outliers, FALSE, 0) 

plot(model3)
plot(model3_extra_analysis)
model3_analysis_result = result_metrics(model3_extra_analysis, alpha=0.10)
model3_analysis_result
```
```
