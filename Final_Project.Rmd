...
This study is conducted by the following members:
...
Of the methods we have tested, we have narrowed our efforts down to using AIC as criteria in selecting a model and comparing both the STEPWISE search method and the EXHAUSTIVE search methods.
...
The following helper function (result_metrics) to capture R2, adjusted R2, RMSE, number of coefficients used, and 
list of coefficients that are significant based of a given alpha.
...
In this study 1, we have captured the time it takes for the search to complete.
...
Re-run the model without the outliers  using the model drived from the aic backward stepwise search.
...
model3_result = result_metrics(model_exhaust, alpha=0.10)
...
Re-run the model without the outliers using the fitted model above
...
Based on the result, it shows that models from Study 2 and Study 3 show similar R2 (`r model3_result$r2`) and 
adjusted R2 (`r model3_result$adjusted_r2`) which are higher than Study 1 (`r model1_result$r2`), and an RMSE (`r model3_result$rmse`) which is  lower than than that of Study 1 (`r model3_result$rmse`).  From that perspective,  model 2 and model 3 are the best. However, from the number of selected variables / coefficients and number of significant coefficients, it shows that model 3 is the best model with coefficient count of (`r model3_result$coef_count`) that are all significant.

However, in terms of significance, what does ANOVA tell us in the F-test.
...
The result of the F-TEST does show that model 1 (Full Hypothesis) is the choice of model since the F-test rejects the null hypothesis. However, if one has to compare all the numeric values, apart from model 1 being slow in aic backward keywise search, there is really not much significan difference in terms of R2, adjusted R3, and RMSE.

But for the sake of argument, we still use model 1 from F-TEST result to compare against model 3.
...
The histograms seems to be the same.
...
Basically, if we are to assume for a moment that the test data all come from our attempts to make bear, it would seem that by following the measuring gadgets ( or sensors ) based on the model, we should be able to closely get an accuracy of `r accuracy`.
...
Notice that residual range droped from (-19, 6) to (-6, to 6)
